...
export function TraceTimeline({ traces }: { traces: Trace[] }) {
  if (traces.length === 0) {
    return <p className="text-sm text-slate-600">No trace records yet.</p>;
  }
  ...
  return (
    <div className="space-y-3">
      {traces.map((trace, index) => {
        ...
        return (
          <div key={`${trace.node_id}-${index}`} className="rounded-md border border-line bg-white p-4">
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <details className="mt-3 text-xs text-slate-700">
                  <summary className="cursor-pointer">Input / Output</summary>
