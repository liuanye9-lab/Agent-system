    ...
    def _deterministic_evals(self, workflow_id: str, target_node_id: str | None = None) -> list[EvalSpec]:
        return [
            EvalSpec(
                ...
                name="新品上市端到端 golden case",
                eval_type=EvalType.END_TO_END,
                target_node_id=None,
                input_case={
                    "product": "AI workflow platform",
                    "market": "global",
                    "goal": "launch readiness decision",
                },
                expected_output={
                    "status": "completed_or_paused_for_approval",
                    "must_include": ["trace", "business_case", "go_no_go_decision"],
                },
            ...
            EvalSpec(
                ...
                target_node_id=target_node_id or "business-case",
                input_case={"context": {"positioning": "clear", "pricing": "draft"}},
                expected_output={"decision": "continue_or_rework", "risks": []},
